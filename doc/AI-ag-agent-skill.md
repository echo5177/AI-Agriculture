...
- 前端协作约定：当前完整前端在 `frontend/`，不是 `cloud/dashboard/`。
...
## 6. 语义边界（必须分清）
- `网关（Gateway）`：设备身份主体，先注册再上报。注册成功的关键 ACK 为 `ack:register_ok`，并发放/校验 `device_key`。
- `传感器（Sensor）`：网关携带的数据通道，不是独立身份主体。传感器数据包必须带 `device_id`，并且 `sensor_id` 必须在该网关注册合同内。
- `未注册网关` 与 `未知传感器` 区分：
  - `ack:unregistered`：`device_id` 未注册，或该设备不允许该 `sensor_id`。
  - `ack:unknown_sensor`：`sensor_id` 在云端规则里根本不存在。
- 图传语义：图传是网关业务扩展链路，不等于“网关注册”也不等于“传感器遥测”。三者状态必须分别展示。
- UI 展示要求：
  - 设备列表按“网关”维度；
  - 指标卡/图表按“传感器字段”维度；
  - 图传结果按“上传记录+推理结果”维度。
