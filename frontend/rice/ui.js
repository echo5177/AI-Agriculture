...
window.UI = (() => {
            ...
            .map((sid) => {
                ...
                <div class="sensor-tile group" onclick="UI.openSensorDetail('${sid}')">
                    ...
                    <div>
                        ...
                        <h3 class="text-xs font-bold text-[#332b21] tracking-wider">${sid.toUpperCase()}</h3>
                        <p class="text-[10px] text-[#332b21]/60 mt-1">${fieldPreview || window.t('no_data')}</p>
                    </div>
                    <div class="flex items-center justify-between mt-1 pt-2 border-t border-white/5">
                        <span class="text-[9px] text-[#332b21]/60">STATUS: ${window.t(isFault ? 'status_fault' : 'status_online')}</span>
    ...
    const openSensorDetail = (sid) => {
        ...
        const rows = Array.from(schema.fields.values())
            .map((f) => {
                ...
                return `
                    <div class="flex items-center justify-between p-4 bg-[#332b21]/5 rounded-xl border border-white/5">
                        <div class="flex items-center gap-3">
                            <div class="w-2 h-2 rounded-full ${isFault ? 'bg-rose-400' : 'bg-emerald-400'}"></div>
                            <span class="text-xs text-[#332b21]/80 font-bold">${f.label}</span>
                        </div>
                        <div class="text-right">
                            <span class="text-sm font-mono text-[#332b21]">${display}</span>
                ...
                <div class="glass-panel p-8 flex flex-col items-center justify-center relative overflow-hidden">
                    ...
                    <div class="w-48 h-48 rounded-full border-8 border-emerald-500/20 flex items-center justify-center relative shadow-[0_0_50px_rgba(16,185,129,0.1)]">
                        <div class="text-center">
                            ...
                            <p class="text-5xl font-black ${isFault ? 'text-rose-400' : 'text-[#332b21]'} tracking-widest">${window.t(isFault ? 'status_fault' : 'status_ok')}</p>
                        </div>
                    ...
                    <h2 class="mt-8 text-2xl font-black text-[#332b21] tracking-widest uppercase">${sid}</h2>
                    <p class="text-xs text-[#332b21]/60 mt-2 font-mono">LATEST_TS: ${formatDate(latest?.ts)}</p>
                    ${reasons.length ? `<p class="text-[10px] text-rose-400 mt-2">${reasons.join(', ')}</p>` : ''}
                </div>
                <div class="space-y-6">
                    <div class="glass-panel p-6">
                        <h3 class="text-sm font-bold text-[#332b21] mb-6 flex items-center gap-2 uppercase tracking-widest"><i class="fa fa-list text-emerald-500"></i>${window.t('field_details')}</h3>
                        <div class="space-y-4">
        ...
        const card =
            state === 'failed'
                ...
                : state === 'inferred'
                    ...
                    : { bg: 'bg-[#332b21]/5', text: 'text-[#332b21]/60', border: 'border-white/5', badge: 'STORED' };
        ...
        return `
            <div class="p-4 border ${card.border} ${card.bg} rounded-xl mb-3">
                <div class="flex justify-between items-start mb-2 gap-2">
                    <p class="text-[10px] text-[#332b21]/60 font-mono">${formatDate(r.captured_at || r.ts)}</p>
    ...
    const setVisionStatus = (message, level = 'idle') => {
        const statusEl = document.getElementById('mobileLiveStatus');
        if (!statusEl) return;
        const palette = {
            idle: 'text-[#332b21]/60',
            loading: 'text-violet-700',
    ...
    const HomePositioning = {
        ...
        populateDropdown: (type, items) => {
            ...
            let html = '';
            if (type === 'location') {
                html += `<div class="px-4 py-2.5 text-[11px] text-[#332b21]/80 hover:bg-emerald-500/10 cursor-pointer flex items-center gap-2 transition-colors" onclick="UI.HomePositioning.selectLocation(null)">
            ...
            items.forEach(item => {
                ...
                html += `<div class="px-4 py-2.5 text-[11px] text-[#332b21]/80 hover:bg-emerald-500/10 cursor-pointer flex items-center gap-2 transition-colors" onclick="UI.HomePositioning.${fn}('${item}')">
        ...
        submit: async () => {
            ...
            try {
                ...
                if (result.upload_id) {
                    ...
                    setVisionStatus(`${window.t('mobile_upload_success')}: ${result.predicted_class || result.upload_id}`, 'success');
                }
    ...
    const LiveCamera = {
        ...
        start: async (preferredDeviceId = null) => {
            ...
            setVisionStatus(window.t('live_camera_starting'), 'loading');
            ...
            try {
                ...
                if (!stream) {
                    throw lastErr || new Error("无法获取视频流，请检查权限。");
                }
        ...
        uploadSnapshot: async () => {
            ...
            try {
                ...
                const result = await window.API.uploadImage({ file, tag, retries: 0 });
                if (result.upload_id) {
                    setVisionImage(`/api/v1/image/file?upload_id=${encodeURIComponent(result.upload_id)}&_=${Date.now()}`);
                }
                return result;
        ...
        captureFile: async (prefix) => {
            const video = document.getElementById('mobileLiveVideo');
            const canvas = document.getElementById('mobileLiveCanvas');
            if (!video || !canvas) throw new Error('camera canvas unavailable');
            canvas.width = video.videoWidth;
            canvas.height = video.videoHeight;
            const ctx = canvas.getContext('2d');
            if (!ctx) throw new Error('camera canvas context unavailable');
        ...
        resolveTag: (captureInput) => {
            const tag = Upload.resolveTag();
            const note = `${tag.farm_note || ''}`;
            const marker = `capture_mode=manual;capture_input=${captureInput}`;
            return {
                ...tag,
                ts: new Date().toISOString(),
                capture_mode: 'manual',
                capture_input: captureInput,
                farm_note: note.includes('capture_mode=')
                    ? note.replace(/capture_mode=[^;|\s]+(;capture_input=[^;|\s]+)?/i, marker)
                    : `${note ? `${note} | ` : ''}${marker}`,
            };
        },
    };
                    ...
                    return `
                        <div class="sector-crop-group">
                            <div class="sector-item ${isActive ? 'active' : ''}" onclick="UI.Charts.toggleCropLocations('${crop}', this)">
                                ...
                                <span class="text-[11px] font-bold text-[#332b21]/80 uppercase tracking-wider flex-1">${crop}</span>
                            ...
                            <div class="sector-locations ${isActive ? '' : 'hidden'}" id="crop-locs-${crop}">
                                ${locs.map(loc => `
                                    <div class="sector-sub-item ${Charts.selectedCrop === crop && Charts.selectedLocation === loc ? 'active' : ''}" onclick="UI.Charts.selectCropLocation('${crop}', '${loc}', this, event)">
                                        ...
                                        <span class="text-[10px] text-[#332b21]/60 font-bold tracking-wider">${loc}</span>
                                    </div>
                                `).join('')}
                            </div>
                        </div>
            ...
            if (sensorList) {
                ...
                sensorList.innerHTML = sids.map(sid => `
                    <label class="sensor-pill cursor-pointer group">
                        ...
                        <span class="text-[10px] text-[#332b21]/60 peer-checked:text-emerald-100 uppercase font-bold">${sid}</span>
                    </label>
                `).join('');
            }
                    ...
                    card.innerHTML = `
                        <div class="flex items-center justify-between mb-8">
                            <div class="flex items-center gap-4">
                                <div class="w-10 h-10 rounded-xl bg-[#332b21]/5 border border-white/10 flex items-center justify-center">
                                    <i class="fa ${sid.includes('soil') ? 'fa-leaf' : 'fa-area-chart'} text-emerald-400"></i>
                                </div>
                                <div>
                                    <h4 class="text-xs font-black text-[#332b21] uppercase tracking-widest">${sid} / ${fieldSpec.label} ${fieldSpec.unit ? `(${fieldSpec.unit})` : ''}</h4>
                                    ...
                                    return `
                                        <div class="aspect-[4/3] bg-black/40 rounded-xl border border-white/5 overflow-hidden group relative cursor-pointer" onclick="UI.openImagePreview('${imgUrl}', '${title.replace(/'/g, '\\\'')}')">
                                            ...
                                            <div class="absolute bottom-2 left-2 right-2 flex justify-between items-center bg-black/40 rounded px-1.5 py-0.5">
                                                <span class="text-[8px] text-[#332b21]/80 font-mono">${formatDate(row.captured_at || row.received_at)}</span>
    ...
    const Health = {
            ...
            container.innerHTML = servers.map(s => `
                <div class="status-card health-${s.status} mb-4">
                    <div class="flex items-center justify-between mb-2">
                        <span class="text-xs font-bold text-[#332b21]">${s.name}</span>
        ...
        renderSensors: () => {
            ...
            container.innerHTML = sensorIds.map(sid => {
                ...
                <div class="status-card health-${status} mb-4">
                    <div class="flex items-center justify-between mb-2">
                        <span class="text-xs font-bold text-[#332b21] uppercase">${sid}</span>
                        <div class="health-dot dot-${status}"></div>
                    </div>
                    <div class="flex flex-col gap-1">
                        <p class="text-[9px] text-[#332b21]/60 font-medium">${reasonText}</p>
                        <div class="h-1 w-full bg-[#332b21]/5 rounded-full overflow-hidden mt-1">
            ...
            renderInstructions: () => {
                ...
                container.innerHTML = UI.AI.instructionList.map((instr, idx) => `
                    <div class="instruction-item flex items-start gap-2 group">
                        <span class="flex-1 text-[11px] text-[#332b21]/60 line-clamp-2 leading-tight">${instr}</span>
